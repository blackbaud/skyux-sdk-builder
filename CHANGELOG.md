...
- Moved the `SkyThemeService` provider from the application's root component to the application module so components created outside of the root component's hierarchy (such as the modal component) can inject the application's `SkyThemeService` instance. [#282](https://github.com/blackbaud/skyux-sdk-builder/pull/282)
