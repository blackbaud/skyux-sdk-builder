...
function generateComponents(skyAppConfig) {
  const srcPath = skyAppConfig.runtime.srcPath;

  const componentsPattern = path.join(srcPath, skyAppConfig.runtime.componentsPattern);
  ...
  const ignorePattern = ignoredDirectories.map(dir => {
    return path.join(srcPath, `./${dir}/**/*`);
  });

  return glob
    .sync(componentsPattern, {
